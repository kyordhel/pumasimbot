...
int get_inputs(int argc, char **argv, Inputs *inputs)
{
    ...
    if (argc>1) {
        for(i=1; i<argc; i++){
            if (!strcmp(argv[i],"-h")){
                printf("Sintax: %s -x x_origen -y y_origen -a angle_robot -v x_destination -z y_destination -s sensor -n num_sensors -t theta_sensor -r range_sensor -p path_data -e environment -radio radio_robot -advance magnitud_advance -MaxAngle max_angle -steps number_steps -largest largest_value_sensors -noise add_noise -selection selection -vq quantize_sensors -size_vq vector_quantizer_size -pr_out random_outputs -nn_rec recurrent_nn -nn_unk num_unknown_obstacles -search search_method\n",argv[0]);
                printf("examples:\n");
                ...
                printf("%s -x 1 -y 2.5 -a 3.1416 -v 2.0 -z 3.1 -s laser -n 4 -t -0.7075 -r 3.1416 -selection 3 -search 2\n",argv[0]);
                exit(1);
            }
                ...
                inputs->method=atoi(argv[i+1]);
            }
            /* Network */
            else if (!strcmp(argv[i],"-address")){
                strcpy(inputs->address,argv[i+1]);
            }
            else if (!strcmp(argv[i],"-port")){
                inputs->port=atoi(argv[i+1]);
            }
        }
    }
    else{
    ...
    printf("%s -x %f -y %f -a %f -v %f -z %f -s %s -n %d -t %f -r %f -p %s -e %s -advance %f -MaxAngle %f -selection %d -largest %f -noise %d -omni %d -vq %d -size_vq %d -pr_out %d -out_file %s -nn_rec %d -nn_unk %d -search %d\n",
        ...
        inputs->pr_out,inputs->output_file,inputs->nn_rec, inputs->nn_unk, inputs->method);
