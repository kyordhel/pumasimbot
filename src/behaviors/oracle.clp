...
(defrule Max-Values
	...
	=>
	(printout t "received max advance rotation" crlf)
	(broadcast "received from max advance rotation")
)
...
(defrule obstacle-left-light-front-left
        ...
        ;; rotate to the right 45 degrees and advance forward
        ;(assert (movement ?num ?right_45 ?forward 0.5))
        ; rotate to the right 45 degrees and goes backward
        (assert (movement ?num ?right_45 ?backward 0.5))
)
...
(defrule arbiter
	...
	(if (> ?intensity 30.0) then
		(printout t "goto " ?num " " ?rotation " " ?advance " 1.0 " crlf)
		(bind ?message (str-cat "goto " ?num " " ?rotation " " ?advance " 1.0"))
        	(broadcast ?message)
	else
		(printout t "goto " ?num " " ?rotation " " ?advance " " ?status crlf)
		(bind ?message (str-cat "goto " ?num " " ?rotation " " ?advance " " ?status))
        	(broadcast ?message)
	)
)
